...
import com.example.canva.analyzer.ZXingBarcodeAnalyzer
import com.example.canva.ui.ScannerResultDialog
...
class BarcodeScanningActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ///add bar code
        val view: View = LayoutInflater.from(this).inflate(R.layout.line, null)
        val paramsFragment = FrameLayout.LayoutParams(
            FrameLayout.LayoutParams.MATCH_PARENT,
            FrameLayout.LayoutParams.MATCH_PARENT
        )
        val paramsRelativeLayout = RelativeLayout.LayoutParams(
        ...
        val animation = AnimationUtils.loadAnimation(
            applicationContext, R.anim.scal_animation
        )
        view.startAnimation(animation)

        // background
        view.background = getDrawable(R.drawable.ic_line)
        //   binding.parentRelative.addView(view, params)


        //  relativeLayout.addView(view, params);
        runOnUiThread {
            Handler().postDelayed({
                binding.overlay.boxRect?.let {
                    paramsFragment.setMargins(
                        it.left.toInt(),
                        it.top.toInt(),
                        it.left.toInt(),
                        it.top.toInt()
                ...
                binding.parentRelative.layoutParams = paramsFragment
                binding.parentRelative.addView(view, paramsRelativeLayout)
            }, 100)
        }
